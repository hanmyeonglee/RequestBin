...
object BinRepository {
  ...
  def fromBinId(binId: String): Option[Bin] = {
    sql"SELECT id, binId FROM bin WHERE binId = ${binId} and lastUsedAt >= datetime('now', '-${secondsToLive} seconds')"
  ...
  def deleteAllExpiredBins: Unit = {
    sql"DELETE FROM bin WHERE lastUsedAt < datetime('now', '-${secondsToLive} seconds')".update.apply()
  }
}
