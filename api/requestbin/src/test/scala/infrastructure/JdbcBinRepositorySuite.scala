...
class JdbcBinRepositorySuite extends FunSuite {
    ...
    test("save and findByBinId return the saved bin") {
        withTx { implicit ctx => binRepo.save(Bin("abc123", 1000L)) }
        val found = withTx { implicit ctx => binRepo.findByBinId("abc123") }
        assertEquals(found, Some(Bin("abc123", 1000L)))
    }

    test("save upserts: second save with same binId overwrites lastUsedAt") {
        withTx { implicit ctx => binRepo.save(Bin("upsert-bin", 1000L)) }
        withTx { implicit ctx => binRepo.save(Bin("upsert-bin", 9999L)) }
        val found = withTx { implicit ctx => binRepo.findByBinId("upsert-bin") }
        assertEquals(found.map(_.lastUsedAtUnixTimeSeconds), Some(9999L))
    }
    ...
    test("deleteAllExpiredBins removes bins with lastUsedAt < threshold and keeps others") {
        withTx { implicit ctx =>
            binRepo.save(Bin("expired-bin", 100L))  // 100 < 500 → deleted
            binRepo.save(Bin("active-bin",  600L))  // 600 >= 500 → kept
        }
        withTx { implicit ctx => binRepo.deleteAllExpiredBins(500L) }

        assertEquals(withTx { implicit ctx => binRepo.findByBinId("expired-bin") }, None)
        assertEquals(withTx { implicit ctx => binRepo.findByBinId("active-bin")  }, Some(Bin("active-bin", 600L)))
    }

    test("deleteAllExpiredBins threshold is exclusive: lastUsedAt == threshold is NOT deleted") {
        withTx { implicit ctx => binRepo.save(Bin("boundary-bin", 500L)) }
        withTx { implicit ctx => binRepo.deleteAllExpiredBins(500L) }  // condition: lastUsedAt < 500
        val found = withTx { implicit ctx => binRepo.findByBinId("boundary-bin") }
        assertEquals(found, Some(Bin("boundary-bin", 500L)))
    }

    test("deleteAllExpiredBins cascades to captured_request rows") {
        withTx { implicit ctx => binRepo.save(Bin("cascade-bin", 100L)) }
    ...
    test("exception inside withTx rolls back - data is not persisted") {
        intercept[RuntimeException] {
            withTx { implicit ctx =>
                binRepo.save(Bin("rollback-bin", 1000L))
